...
public class MqttConfig {
    ...
    public MessageHandler mqttOutbound(MqttPahoClientFactory mqttClientFactory, MqttProperties mqttProperties, @Qualifier("mqttClientId") String mqttClientId) {
        ...
        messageHandler.setDefaultTopic(mqttProperties.getServerTopic());
        return messageHandler;
    }

    @Bean
    public IntegrationFlow mqttInbound(MqttPahoClientFactory mqttClientFactory, MqttProperties mqttProperties, GatewayMessageHandler messageHandler) {
        var sensorTopics = mqttProperties.getSensorTopics();
        return IntegrationFlow.from(
                        new MqttPahoMessageDrivenChannelAdapter("mqtt-sensors", mqttClientFactory, sensorTopics.toArray(new String[0])))
